    ...
    ******************************************/
	...
	private function setLanguage($language) {
		$this->language = $language;
			include('inc/languages/'.$language.'.php');
		$this->lang = $lang;
	}
	...
	private function attach() {
	    ...
	    $this->Cell(0,5,iconv("UTF-8", "ISO-8859-1",strtoupper("Adjunto")),0,1,'C');
		$this->SetFont($this->font,'',9);
		$this->Ln(5);


		$this->Image($this->attach, null, null, 180, 0);
	...
	public function Header() {
		...
		if($this->PageNo()== 1) {
			...
			if($this->display_tofrom === true) {
				$this->Cell($width,$lineheight,iconv("UTF-8", "ISO-8859-1",strtoupper($this->lang['from'])),0,0,'L');
	...
	public function Body() {	
		...
		if($this->items) {
			foreach($this->items as $item) 
			{
				...
				$x = $this->GetX();
				$this->Cell($this->firstColumnWidth,$cHeight,iconv("UTF-8", "ISO-8859-1",htmlentities($item['item'])),0,0,'L',1);
				if($item['description']) {
					$resetX = $this->GetX();
					$resetY = $this->GetY();
	...
	public function Footer() {
		...
		$this->Cell(0,10,iconv("UTF-8", "ISO-8859-1",$this->lang['page']).' '.$this->PageNo().' '.iconv("UTF-8", "ISO-8859-1",$this->lang['page_of']).' {nb}',0,0,'R');
	}
